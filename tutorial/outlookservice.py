...
def get_my_messages(access_token):
  ...
  #print("himanshu adding line below")
  # print(r.content)
  #print(r.json())
  if (r.status_code == requests.codes.ok):
    return r.json()
...
def get_my_contacts(access_token):
  ...
  if (r.status_code == requests.codes.ok):
    ...
    return "{0}: {1}".format(r.status_code, r.text)
  #####################Test BEgins here################
  #Adding a function to get the feeds
def get_my_feeds(access_token):
      #get_feeds_url = graph_endpoint.format('/me/feeds')
      get_feeds_url = graph_endpoint.format('/auditLogs/signIns')
      # Use OData query parameters to control the results
      #  - Only first 10 results returned
      #  - Only return the GivenName, Surname, and EmailAddresses fields
      #  - Sort the results by the GivenName field in ascending order
      query_parameters = {'$top': '10',
                          ...
                          } #shift it up in case of format error

      r = make_api_call('GET', get_feeds_url, access_token, parameters=query_parameters)
      print("himanshu adding line below")
      # print(r.content)
      print(r.json())
